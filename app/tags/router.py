...
def save_user_tags(user_id: int, tags_input: TagsInput, conn=Depends(get_session)):
    ...
    user_exists = conn.execute(user_exists_query).fetchone()
    if not user_exists:
...
def get_user_tags(user_id: int, conn=Depends(get_session)):
